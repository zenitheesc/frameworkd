...
DBusHandler::DBusHandler(const std::string &serviceName) : _isServer{true}, _serviceName{serviceName}
{

    _connection = sdbus::createSystemBusConnection(serviceName);
}

DBusHandler::DBusHandler(const std::string &serviceName, bool isServer) : _isServer{isServer}, _serviceName{serviceName}
{
